...
class Validate {
    ...
    Validate_Name = () => {
        ...
        if(name[0].value==="")
            message.innerText="Kindly enter the Name"
    ...
    Validate_Address = () => {
        ...
        if(address[0].value==="")
            ...
            message.textContent="Address can have alphabets, numbers, space and the following special characters /,:-."
        else
            message.textContent=""
    }
    ...
    Validate_Email = () => {
        ...
        if(email[0].value==="")
            ...
            message.textContent="Invalid mail address. Mail address can have alphabets, numbers, .(dot), _(underscore) and @"
        else
            message.textContent=""
    }
    ...
    Validate_Password = () =>{
        ...
        if(password[0].value==="")
            ...
            message.textContent="Password should have atleast 1 upper case, 1 lower case, 1 number and 1 special character !@#$%^&*()"
        else
            message.textContent=""
        ...
        if(check)
            {
                ...
                else if(password1[0].value.normalize()!==password2[0].value.normalize())
                    message.textContent="Password and confirm password must be same"
                else
                    message.textContent=""
            }
        else
            message.textContent="Kindly fill the Password first!!"
    }
